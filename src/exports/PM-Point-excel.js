...
// @ts-ignore
const { Project, RuleDefinition, UserMaster } = require('../models');
...
module.exports = asyncHandler(async (req, res, next) => {
    try {
        ...
        const Approver_List = await UserMaster.findAll({
            attributes: ['Account'],
            where: {
                DepartmentID: DepartmentID,
                Status: 1,
        ...
        const approverList = workbook.addWorksheet('Approver_List', { state: 'hidden' });
        //
        ...
        // show rule point
        requestPoint.fillFormula('C2:C9999', '=IF(TRIM(B2)="","",IF(TRIM(B2)="Other","",VLOOKUP(B2,Rule_Definition!$A$2:$B$44,2,FALSE)))');
        ...
        requestPoint.dataValidations.add('F2:F9999', {
            type: 'list',
            allowBlank: true,
            formulae: [Approver_List.length !== 0 ? `=Approver_List!$A$2:$A$${Approver_List.length + 1}` : '"NO APPROVER"'],
        });

        // month
        // @ts-ignore
        requestPoint.dataValidations.add('H2:H9999', {
            type: 'list',
            allowBlank: true,
        ...
        // total-point
        requestPoint.fillFormula('J2:J9999', '=IF(TRIM(B2)="", "",IF(TRIM(B2)="Other","", C2*G2))');

        //

        res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        res.setHeader('Content-Disposition', 'attachment; filename=' + 'PM-POINT-EXCEL.xlsx');
