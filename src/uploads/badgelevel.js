...
const BadgeLevel = require('../services/badgelevel.service');
const { errorResponse, successResponse } = require('../libs/response');

module.exports = {
    uploadBadgeLevelImage: multer({
        limits: {
    ...
    removeBadgeLevelImage: asyncHandler(async (req, res, next) => {
        if (req.file != undefined) {
            const badge = await BadgeLevel.fncFindOne(req);
            if (badge) {
    ...
    updateBadgeLevelImage: asyncHandler(async (req, res, next) => {
        const pathSaveBadgeImg = `/public/badge/${req.file.filename}`;
        req.body.ImageURL = pathSaveBadgeImg;

        const update = await BadgeLevel.fncUpdateOne(req, next);
